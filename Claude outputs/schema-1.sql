...
begin
  ...
  insert into threads (title, creator_id, creator_token_hash, created_at, last_reply_at, reply_count, is_archived, is_deleted)
  values (left(trim(p_title), 100), p_author_id, p_author_token_hash, v_now, v_now, 1, false, false)
  returning id into v_thread_id;

  insert into replies (thread_id, number, author_id, author_token_hash, content, created_at, like_count, is_deleted)
  values (v_thread_id, 1, p_author_id, p_author_token_hash, left(trim(p_content), 2000), v_now, 0, false);
...
begin
  ...
  insert into replies (thread_id, number, author_id, author_token_hash, content, created_at, like_count, is_deleted)
  values (p_thread_id, v_next_number, p_author_id, p_author_token_hash, left(trim(p_content), 2000), v_now, 0, false);
...
grant execute on function create_thread(text, text, text, text) to anon, authenticated;
grant execute on function add_reply(bigint, text, text, text) to anon, authenticated;
